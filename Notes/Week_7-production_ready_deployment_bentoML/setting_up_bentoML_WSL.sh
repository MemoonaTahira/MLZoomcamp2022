...
# workflow: 
    ...
    # save model via bento ML and serve it locally
...
# if you plan on installing bentoml with conda, install xgboost with conda too and uncomment the next block and run it:
`
...
# but it's better to install bentoml via pip, however in this case, we need 
...
# create a bentfile.yaml from vscode, initially populated with standard content
...
#### Run the docker to serve the bento and make predictions!
